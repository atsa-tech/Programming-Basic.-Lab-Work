...
Идентичность объектов говорит о том, что две переменные ссылаются на одну и ту же ячейку памяти.  
...
##### Я этого не знал, отсюда и расхождение между прогнозом и реальным выводом.
...
1. / используется для деления с плавающей точкой, чтобы дать максимально точный результат при нецелочисленном делении (с остатком). Он всегда вернет объект типа $\color{red}{\text{float}}$
...
1. $\color{purple}{\text{len()}}$ для обычной строки показывает количество символов в ней, а для байтовой строки считает количество байт, выделенных для кодировки.
